...
Förbereda data handlar ofta om att konvertera olika datavärden till samma skala, men man måste kolla så att inte skalan har betydsle. Även partiskhet behöver analyseras då det påverkar modellen. 

Exempel på variabler map husförsäljning: 
Responsvariabel  
    Slutpris
...
För att undersöka om några variabler korrelerar med varandra och därmed blir signifikanta utan att vara signifikanta kan man göra en scatterplott. En scatterplott kan visa om det verkar finnas ett linjärt samband mellan variablerna. I studien hittades ett linjärt samband mellan boarea och rum. Då ersattes variabeln Rum med variabeln Borum. Definitionen av Borum - boarea / antal rum.
...
Sen får man välja vilken nivå på normalfördelningen man vill avstå på. I studien valde man bort 5% nivån och då försvann alla utom tre variabler. Variablerna visade sig vara Boarea, Byggår och Sålt. Modellen ser nu ut som följande 
log (α + β1 * boarea + β2 * byggår + β3 * sålt + ϵ)
ε är feltermen (error term), som står för den del av variationen i y som inte kan förklaras av modellen. 

I normalfördelningsplotten till denna modell visar sig punkterna vara kring den raka linjen i figuren. Man får sedan studera residualplottarna igen för de enskilda variablerna för att se om några punkter sticker ut och som ska tas bort.
